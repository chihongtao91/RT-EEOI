...
,where the sequence of arguments after the script name are: the username of a grosstonnage.com account and its password. The account name and password above are temporary and would have expired.
...
In order to access the database using PostgreSQL, you will need to install the psycopg2 module. User will also have to replace the database name (dbname) and password with their own in the grossTonnageParse.py script. In our database design, the positional and static messages are stored on the local machine with the table name position_msg and static_msg. The name of the ship information table is ship_info.  
